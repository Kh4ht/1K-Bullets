...
namespace SmallScale.FantasyKingdomTileset.AbilitySystem
{
    ...
    [System.Serializable]
    [AbilityComponentDescription("Fires a configurable volley of projectiles toward the desired direction, target, or mouse aim.")]
    [MovedFrom("AbilitySystem")]
    public sealed class ProjectileVolleyStep : AbilityStep
    {
...
#pragma warning restore 0414
        ...
        public override IEnumerator Execute(AbilityRuntimeContext context)
        {
            ...
            {
                FireProjectile(prefab, spawnTransform, basePosition, baseZ, baseDirection, projectileSpeed, projectileDamage, projectileLife, hitMask, context.Transform);
            }
            else
            {
                switch (spreadMode)
                {
                    case SpreadMode.EvenCone:
                        FireEvenCone(prefab, spawnTransform, basePosition, baseZ, baseDirection, projectileSpeed, projectileDamage, projectileLife, hitMask, context.Transform, count);
                        break;
                    case SpreadMode.RandomCone:
                        FireRandomCone(prefab, spawnTransform, basePosition, baseZ, baseDirection, projectileSpeed, projectileDamage, projectileLife, hitMask, context.Transform, count);
                        break;
                }
            }
        ...
        void FireEvenCone(Projectile2D prefab, Transform spawnTransform, Vector2 basePosition, float baseZ, Vector2 baseDirection,
            float speed, int damage, float life, LayerMask mask, Transform owner, int count)
        {
            float totalAngle = Mathf.Clamp(coneAngleDegrees, 0f, 360f);
            if (count == 1 || totalAngle <= 0.01f)
            {
                FireProjectile(prefab, spawnTransform, basePosition, baseZ, baseDirection, speed, damage, life, mask, owner);
                return;
            }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                FireProjectile(prefab, spawnTransform, basePosition, baseZ, dir, speed, damage, life, mask, owner);
            }
        }
